...
const terminal_module = require('./terminal_module.js'); //main class
...
var WIDE = {
  ...
  settings: {
    ...
    //change it if it is hosted in a different folder than the index.html
    persistent_session: true,
    //set to false if you do not want the keys to be stored in localStorage
    plugins: []
  },
  //globals
  key: "KEY",
  ...
  init: function () {
    ...
    window.onbeforeunload = function () {
      WIDE.saveSession();
      if (!WIDE.checkAllFilesSaved()) return "Not all files are saved";
    };
    ...
    terminal_module.initTerminal();
  },
  onResize: function () {
    for (var i = 0; i < this.files.length; ++i) if (this.files[i].editor) this.files[i].editor.layout();
  },
  onReady: function () {
    WIDE.reset();
    WIDE.loadSession(); // Terminal.applyAddon(attach);
  ...
  showCodeEditor: function (file_info, force_assign) {
    if (!file_info) return null; //editor already exist

    if (file_info.editor) {
      if (this.visible_file != file_info) {
        if (this.visible_file) this.visible_file.editor_element.style.display = "none";
      }

      file_info.editor_element.style.display = "";
      this.visible_file = file_info;
      if (force_assign) file_info.editor.setValue(file_info.content);
    ...
    file_info.editor_element = editor_element; //a model is a file content

    var model = monaco.editor.createModel("", "javascript");
    file_info.model = model; //an editor is a view of a model
    ...
    editor.file_info = file_info; //events
    ...
    editor.addCommand(monaco.KeyCode.Escape, function () {//console.log('my command is executing!');
    }); //hide previous editor

    if (this.visible_file && this.visible_file != file_info) this.visible_file.editor_element.style.display = "none";
    this.visible_file = file_info; //update content and settings

    editor.setValue(file_info.content || "");
    if (file_info.cursor) editor.setPosition(file_info.cursor);
    if (file_info.scrollTop) editor.setScrollTop(file_info.scrollTop);
    return editor;
  },
  load: function (filename, callback, open, focus) {
    var file_info = this.onFileAdded(filename);
    console.log("loading file: " + filename);
    if (!WIDE_SERVER.load(filename, inner_loaded, inner_error)) return null;

    function inner_loaded(filename, data) {
      var file_info = WIDE.onFileLoaded(filename, data);
      if (callback) callback(filename, file_info);
      if (open) WIDE.open(filename, focus);
    }
  ...
  save: function () {
    ...
    if (!WIDE_SERVER.save(filename, content, inner_complete, inner_error)) return;
    ...
    function inner_error() {
      ...
      setTimeout(function () {
        document.querySelector("#code-area").classList.remove("error-saving");
      }, 1000);
    }
  },
  "delete": function (filename) {
    ...
    if (!WIDE_SERVER.delete(filename, inner_complete)) return;
  ...
  move: function (filename, new_filename) {
    if (!filename || !new_filename) return;
    ...
    var file_info = WIDE.files_by_name[filename];
    if (file_info) this.close(filename);
    if (!WIDE_SERVER.move(filename, new_filename, inner_complete)) return;
  ...
  open: function (filename, focus) {
    if (!filename) return;
    if (this.current_file && this.current_file.name == filename) return;
    ...
    var ext = "text";
    if (index != -1) ext = filename.substr(index + 1).toLowerCase();
    if (this.extensions_to_language[ext]) ext = this.extensions_to_language[ext];
    monaco.editor.setModelLanguage(file_info.model, ext);
    if (focus) file_info.editor.focus();
  },
  close: function (filename) {
    var is_current = this.current_file && this.current_file.name == filename;
    if (!filename) filename = this.current_file ? this.current_file.name : null;
    if (!filename) return;
    var file_info = this.files_by_name[filename];
    if (!file_info) return; //remove filename from list

    file_info.file_element.parentNode.removeChild(file_info.file_element); //update numbers next to filename

    var entries = document.querySelectorAll("#open-files .filename .number");

    for (var i = 0; i < entries.length && i < 10; ++i) entries[i].innerHTML = i + 1; //remove file from containers


    var index = this.files.indexOf(file_info);
    if (index != -1) this.files.splice(index, 1);
    delete this.files_by_name[filename];
    if (file_info.onLeave) file_info.onLeave(file_info.editor ? file_info.editor.getValue() : null); //remove editor
    ...
    if (!next_file) {
      ...
      header.innerHTML = "";
    } else this.open(next_file.name);
  },
  create: function (filename, content, open) {
    if (!filename) filename = prompt("Choose file name");
    if (!filename) return;
    if (this.current_file && this.current_file.name == filename) return;
    this.onVisibleFileChange();
    ...
    if (open) this.open(filename, true);
    return file_info;
  },
  reset: function () {
    this.files = [];
  ...
  list: function (folder, skip_log, on_complete) {
    ...
    queryforEach("#sidebar .header button.sidebarmode", function (a) {
  ...
  autocomplete: function (filename, on_complete, folder) {
    ...
    if (!WIDE_SERVER.autocomplete(fullpath, inner_complete)) return;

    function inner_complete(data, shared) {
      if (on_complete) on_complete(data, shared);
    }
  },
  serialize: function () {
    ...
    for (var i = 0; i < this.files.length; ++i) {
      ...
      if (!file_info.name || file_info.internal) continue;
      var item = {
        name: file_info.name,
        cursor: file_info.cursor,
        scrollTop: file_info.scrollTop
      };
      if (file_info._exist === false) item.content = file_info.editor ? file_info.editor.getValue() : file_info.content;
      o.files.push(item);
    }

    if (this.current_file) o.current = {
  ...
  configure: function (o) {
    ...
    if (o.settings) {
      ...
      if (this.settings.plugins && this.settings.plugins.length) this.loadPlugins();
    }

    if (o.current_folder) this.current_folder = o.current_folder;
    if (o.files) for (var i = 0; i < o.files.length; ++i) {
      var file_data = o.files[i];
      var file_info;
      if (file_data.content != null) file_info = this.create(file_data.name, file_data.content);else file_info = this.load(file_data.name);
      if (!file_info) //no key
        continue;

      for (var j in file_data) file_info[j] = file_data[j];
    }
    ...
    if (o.current) {
      ...
      if (o.current.name) {
        if (o.current.content) file_info = this.create(o.current.name, o.current.content);else file_info = this.open(o.current.name);
      }

      if (file_info) {
        if (o.current.cursor) file_info.editor.setPosition(o.current.cursor);
        if (o.current.scrollTop) file_info.editor.setScrollTop(o.current.scrollTop);
      }
    }

    if (this.current_file && this.current_file.editor) this.current_file.editor.focus();
  },
  ...
  checkAllFilesSaved: function () {
    for (var i = 0; i < this.files.length; ++i) {
      ...
      if (file._exist === false || file.file_element.classList.contains("modifyed")) return false;
    }
  ...
  execute: function () {
    ...
    if (this.current_file.onExecute) return this.current_file.onExecute(content);
  ...
  setKey: function (key, temporal) {
    this.key = key;
    if (this.key) this.list();
    if (temporal) WIDE.settings.persistent_session = false;
    console.log("key assigned");
  },
  //events
  onCommand: function (cmd, skip_console) {
    if (!skip_console) this.toConsole("] " + cmd, "log me");
    ...
    var r = func(cmd, t);
    if (r && r.constructor === String) console.log(r);
  },
  //this is executed before any other onKey event (even inputs or monaco-editor)
  onKey: function (e) {
    //console.log(e);
    if (e.code == "KeyS" && e.ctrlKey) this.save();else if (e.code == "KeyO" && e.ctrlKey) this.toggleConsole();else if (e.code == "KeyQ" && e.ctrlKey) {
      if (document.activeElement == this.console_input) this.current_file.editor.focus();else this.console_input.focus();
    } else if ((e.code == "KeyP" || e.code == "Enter") && e.ctrlKey) this.execute();else if (e.keyCode >= 49 && e.keyCode <= 58 && e.altKey && e.ctrlKey) {
      var file_info = this.files[e.keyCode - 49];
      if (file_info) this.open(file_info.name);
    } else return true; //release the event so monaco can process it

    e.preventDefault();
    e.stopPropagation();
    e.stopImmediatePropagation();
  },
  onEditorKey: function (e) {//console.log("ed key",e);
  },
  onFileAdded: function (filename, content) {
    ...
    if (content != null) file_info.content = content;
    ...
    if (!file_info.file_element) {
      ...
      if (file_info.content === null) element.className += " notloaded";
      ...
      element.querySelector(".close").addEventListener("click", function (e) {
        ...
        if (is_saved || confirm("File not saved, are you sure you want to " + (e.shiftKey ? "delete" : "close") + " it?\nData will be lost.")) {
          if (e.shiftKey) WIDE.delete(element.dataset["filename"]);else WIDE.close(element.dataset["filename"]);
        }
      ...
      for (var i = 0; i < entries.length && i < 10; ++i) entries[i].innerHTML = i + 1;
    }
  ...
  onFileLoaded: function (filename, content) {
    var file_info = this.files_by_name[filename];
    if (!file_info) return null; //a file arrives but is no longer in the editor

    if (!file_info.file_element) throw "file entry without element?";
    file_info.file_element.classList.remove("notloaded");
    file_info.content = content;
    file_info._exist = true;
    if (this.current_file && this.current_file.name == filename) file_info.editor.setValue(content);
    return file_info;
  },
  onShowFolderFiles: function (folder, files, project, skip_log) {
    ...
    container.querySelector(".trash").addEventListener("click", function (e) {
      if (confirm("Are you sure you want to remove the key to this project?")) WIDE.setKey("");
      WIDE.list();
    });
    ...
    for (var i = 0; i < tree.length; ++i) folders.push({
      name: tree[i],
      is_dir: true,
      is_parent: true,
      is_current: i == tree.length - 1,
      fullpath: tree.slice(0, i + 1).join("/")
    });

    var files_and_folders = folders.concat(files);
    var folders = files_and_folders.filter(function (a) {
      return a.is_dir;
    });
    var files = files_and_folders.filter(function (a) {
      return !a.is_dir;
    });
    ...
    for (var i = 0; i < files_and_folders.length; ++i) {
      ...
      var element = document.createElement("div");
      element.className = "filename";
      if (file.is_dir) element.classList.add("folder");
      if (file.is_parent) element.classList.add("parent-folder");
      if (file.is_current) element.classList.add("current-folder");
      var icon = "bootstrap-file";
      if (file.is_dir) icon = "bootstrap-folder-close";
      if (file.is_parent) icon = "bootstrap-play";
      element.innerHTML = '<svg class="icon"><use xlink:href="#si-' + icon + '" /></svg> ' + file.name;
      if (this.files_by_name["/" + fullpath]) element.classList.add("open");
      container.appendChild(element);
      ...
      if (file.mime_type) {
        ...
        if (!can_be_opened) element.classList.add("blocked");
      }

      element.setAttribute("title", fullpath);
      if (can_be_opened) element.addEventListener("click", function (e) {
        if (this.dataset["is_dir"] == "true") WIDE.list("/" + this.dataset["fullpath"]);else {
          if (WIDE.files_by_name["/" + this.dataset["fullpath"]]) {
            if (!confirm("File already open, changes will be lost, are you sure?")) return;
          }
  ...
  onFileSaved: function (filename) {
    var file_info = this.files_by_name[filename];
    if (!file_info) throw "cannot call onFileSaved if there is no file entry";
    if (file_info.file_element) file_info.file_element.classList.remove("modifyed");
    file_info._exist = true;
  },
  onVisibleFileChange: function () {
    if (!this.current_file) return;
    if (this.current_file.onLeave) this.current_file.onLeave(this.current_file.editor ? this.current_file.editor.getValue() : null);
    this.current_file.file_element.classList.remove("selected");
  },
  onContentChange: function (file_info) {
    if (!file_info) return;
    file_info.file_element.classList.add("modifyed");
    this.editor_header.classList.add("modifyed"); //change title too?
  },
  cleanPath: function (path) {
    ...
    for (var i = 0; i < t.length; i++) {
      if (t[i] == "..") result.pop();else result.push(t[i]);
    }

    return result.join("/");
  },
  processFolder: function (folder, default_to_root) {
    if (!folder) return default_to_root ? "./" : this.current_folder;
...
var WIDE_SERVER = {
  ...
  load: function (filename, callback, callback_error) {
    ...
    fetch(WIDE.settings.server_url, info).then(function (resp) {
      ...
      if (callback) callback(filename, data);
    }).catch(function (err) {
      console.error("file not found: " + filename);
      if (callback_error) callback_error(filename);
    });
    return true;
  },
  save: function (filename, content, callback, callback_error) {
    ...
    fetch(WIDE.settings.server_url, info).then(function (resp) {
      ...
      if (r.status == 1) {
        ...
        console.error(r.msg);
        if (callback_error) callback_error(filename, r.msg);
      }
    });
  },
  ...
  move: function (filename, new_filename, callback, callback_error) {
    if (!filename || !new_filename) return false;
    ...
    fetch(WIDE.settings.server_url, info).then(function (resp) {
      ...
      if (r.status == 1) {
        if (callback) callback(filename, new_filename);
      } else {
        console.error(r.msg);
        if (callback_error) callback_error(filename, new_filename);
      }
    });
    return true;
  },
  list: function (folder, skip_log, callback, callback_error) {
    ...
    fetch(WIDE.settings.server_url, info).then(function (resp) {
      ...
      if (r.status == 1) {
        if (callback) callback(r.folder, r.files, r.project, skip_log);
      } else {
        console.error(r.msg);
        if (callback_error) callback_error(r.msg);
      }
    });
  },
  autocomplete: function (fullpath, callback, callback_error) {
    if (!WIDE.key) return false;
    ...
    fetch(WIDE.settings.server_url, info).then(function (resp) {
      ...
      if (r.status == 1) {
        var shared = "";
        if (r.data.length == 1) shared = r.data[0];else if (r.data.length > 1) shared = sharedStart(r.data);
        if (callback) callback(r.data, shared.substr(filename.length));
      } else {
        console.error(r.msg);
        if (callback_error) callback_error(r.msg);
      }
...
WIDE.commands.open = WIDE.commands.load = function (cmd, t) {
  WIDE.load(WIDE.current_folder + "/" + t[1], null, true);
};

WIDE.commands.save = function (cmd, t) {
  WIDE.save();
};

WIDE.commands.new = function (cmd, t) {
  WIDE.create(t[1], "", true);
};

WIDE.commands.rm = WIDE.commands.delete = function (cmd, t) {
  WIDE.delete(WIDE.current_folder + "/" + t[1]);
};

WIDE.commands.mv = WIDE.commands.move = function (cmd, t) {
  WIDE.move(WIDE.current_folder + "/" + t[1], WIDE.current_folder + "/" + t[2]);
};

WIDE.commands.ls = WIDE.commands.list = function (cmd, t) {
  WIDE.list(t[1]);
};

WIDE.commands.close = function (cmd, t) {
  WIDE.close(t[1]);
};

WIDE.commands.reset = function (cmd, t) {
  WIDE.reset();
};

WIDE.commands.execute = function (cmd, t) {
  WIDE.execute();
};

WIDE.commands.reload = function (cmd, t) {
  for (var i in WIDE.files) WIDE.load(WIDE.files[i].name);
};

WIDE.commands.key = function (cmd, t) {
  WIDE.setKey(t.slice(1).join(" "));
};

WIDE.commands.tempkey = function (cmd, t) {
  WIDE.setKey(t.slice(1).join(" "), false);
};

WIDE.commands.pwd = function (cmd, t) {
  WIDE.toConsole(WIDE.current_folder + "/", "filename folder");
};

WIDE.commands.toggleFiles = function (cmd, t) {
  WIDE.toggleFiles();
};

WIDE.commands.settings = function (cmd, t) {
  WIDE.editSettings();
}; //buttons
...
WIDE.buttons.push({
  name: "save current",
  icon: "bootstrap-import",
  command: "save"
});
WIDE.buttons.push({
  name: "settings",
  icon: "bootstrap-menu-hamburger",
  command: "settings"
}); //helpers
...
function headersToObject(headers) {
  var array = Array.from(headers.entries());
  var r = {};

  for (var i = 0; i < array.length; ++i) r[array[i][0]] = array[i][1];
...
},{"socket.io-client":37,"xterm":55,"xterm-addon-attach":53,"xterm-addon-fit":54}],3:[function(require,module,exports){
...
},{"base64-js":7,"buffer":10,"ieee754":32}],11:[function(require,module,exports){
/**
 * Slice reference.
 */
...
},{"./transport":16,"./transports/index":17,"component-emitter":12,"debug":23,"engine.io-parser":26,"indexof":33,"parseqs":34,"parseuri":35}],16:[function(require,module,exports){
/**
 * Module dependencies.
 */
...
},{"../transport":16,"component-inherit":13,"debug":23,"engine.io-parser":26,"parseqs":34,"xmlhttprequest-ssl":22,"yeast":56}],21:[function(require,module,exports){
(function (Buffer){
/**
 * Module dependencies.
 */
...
},{"../transport":16,"buffer":10,"component-inherit":13,"debug":23,"engine.io-parser":26,"parseqs":34,"ws":9,"yeast":56}],22:[function(require,module,exports){
...
},{"./common":24,"_process":36}],24:[function(require,module,exports){
...
},{"./keys":27,"./utf8":28,"after":3,"arraybuffer.slice":4,"base64-arraybuffer":6,"blob":8,"has-binary2":29}],27:[function(require,module,exports){
...
},{"buffer":10,"isarray":30}],30:[function(require,module,exports){
...
},{"./on":39,"./socket":40,"backo2":5,"component-bind":11,"component-emitter":12,"debug":42,"engine.io-client":14,"indexof":33,"socket.io-parser":47}],39:[function(require,module,exports){
...
},{"./on":39,"component-bind":11,"component-emitter":12,"debug":42,"has-binary2":29,"parseqs":34,"socket.io-parser":47,"to-array":52}],41:[function(require,module,exports){
...
},{"./common":43,"_process":36,"dup":23}],43:[function(require,module,exports){
...
},{"./binary":46,"./is-buffer":48,"component-emitter":12,"debug":49,"isarray":44}],48:[function(require,module,exports){
(function (Buffer){
